...
class CallGraphVisitor(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        ...
        for import_item in node.names:
            ...
            to_node = self.get_node(
                '', tgt_name, node, flavor = Flavor.IMPORTEDITEM
            )
            ...
            tgt_module = self.get_node(
                '', mod_name, node, flavor = Flavor.MODULE
            )
    ...
    def visit_Attribute(self, node):
        ...
        self.logger.debug(
            'Attribute %s of %s in context %s'
            % (node.attr, objname, type(node.ctx))
        )
    ...
    def visit_AnnAssign(self, node):
        self.visit_Assign(
            self, node
        )  # TODO: alias for now; add the annotations to output in a future version?
