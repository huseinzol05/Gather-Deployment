    ...
    def __init__(self, graph_params):
        ...
        if g_params['optimizer'] == 'RMSProp':
            self.optimizer = tf.train.RMSPropOptimizer(
                g_params['learn_rate']).minimize(self.cost)
    ...
    def train(self, X, Y, X_val, Y_val, epoch, batch_size):
        ...
        for i in range(1, epoch + 1):
            ...
            for batch in range(num_batches):
                ...
                feed_dict = {self.X: X[bot_idx:top_idx,
                                       :], self.Y: Y[bot_idx:top_idx, :]}
            ...
            print(
                'epoch %d, total batch %d, train loss %f, val loss %f' %
                (i, batch_count, avg_loss, val_loss))
