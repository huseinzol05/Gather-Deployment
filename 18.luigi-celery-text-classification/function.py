...
class Emotion(luigi.Task):
    ...
    def run(self):
        ...
        for i in range(0, len(outputs), self.batch_size):
            ...
            batch_x_text = [i['text'] for i in batch_x_text]
            batch_x_text = [clearstring(t) for t in batch_x_text]
            ...
            labels = [emotion_label[i] for i in np.argmax(output_emotion, 1)]
            for no, label in enumerate(labels):
                outputs[no]['emotion_label'] = label
...
class Save_to_Elastic(luigi.Task):
    ...
    def run(self):
        ...
        for i in range(0, len(emotions), self.batch_size):
            batch = emotions[
                index : min(i + self.batch_size, len(emotions))
            ]
            actions = [
                {
                    '_index': self.index,
                    '_type': 'text',
                    '_id': batch[j]['text'] + self.summary,
                    '_source': batch[j],
                }
